...
int main(int ac, char **av) {
    ...
    while (read(0, &input, 1) > 0) {
        ...
        else                                            valid_input = 0; // Ignores \n
        
        if (valid_input && draw_mode) map[cursor_y * width + cursor_x] = '0';
    }
    ...
    while (iterations-- > 0) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                int neighbors = 0;
                // Count exact neighbors 
                for (int dy = -1; dy <= 1; dy++) {
                    for (int dx = -1; dx <= 1; dx++) {
                        if (!dx && !dy) continue; // Skip self
                        int nx = x + dx, ny = y + dy;
                        if (nx >= 0 && nx < width && ny >= 0 && ny < height && map[ny * width + nx] == '0') 
                            neighbors++;
                    }
                }
                // Dead or Alive Rule
                next_map[y * width + x] = (neighbors == 3 || (neighbors == 2 && map[y * width + x] == '0')) ? '0' : ' ';
            }
        }
        // Swap arrays instantly
        char *temp = map; map = next_map; next_map = temp;
    }
